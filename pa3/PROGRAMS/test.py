...
def testReader():
    ...
    body = reader.BodyLEDs("Problem3-BodyA.txt") 
    bodyB = reader.BodyLEDs("Problem3-BodyB.txt") 
    mesh = reader.SurfaceMesh("Problem3MeshFile.sur") 
    sample_readings = reader.SampleReadings("PA3-A-Debug-SampleReadingsTest.txt")
    assert (body.N_markers == 6)
    assert mesh.N_vertices == 1568
    assert mesh.N_triangles == 3135
    assert sample_readings.N_s == 16 
    assert sample_readings.N_samples == 15
