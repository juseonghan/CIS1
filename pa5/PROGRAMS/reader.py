...
class SampleReadings:
    ...
    def __init__(self, path: str):
        ...
        with open(filename, "r") as f:
            ...
            self.N_s = int(line_comma[0])
            self.N_samples = int(line_comma[1])
            self.N_modes = int(line_space[3])
...
class AtlasModes:
    ...
    def __init__(self, path: str):
        ...
        with open(filename, "r") as f:
            ...
            self.Nvertices = int(first_line[1].split('=')[1])
            self.Nmodes = int(first_line[2].split('=')[1])
            
        self.data = []
        for i in range(self.Nmodes + 1):
            arr = np.loadtxt(filename, delimiter=",", skiprows=1+(i*self.Nvertices + i + 1), max_rows=self.Nvertices, dtype=np.float64)
